...
const AdvocateToDoListComplete = () => {
  ...
  toDo.map((item, index) => {
    if (item?.completed === true) {
      completeTodo.push({
        ...
        createdTime: moment().format('lll', item?.createdTime),
        completedTime: moment().format('lll', item?.completedTime),
        id: item?.id,
      });
    }
  });
            ...
            return (
              <>
                <div className='flex justify-start items-center'>
                  ...
                  >
                    <span className='bg-pink-900 text-white font-semibold w-20 py-1 rounded flex justify-center items-center cursor-pointer'>
                      Pending
                    </span>
                  </div>
                </div>
...
export default AdvocateToDoListComplete;
