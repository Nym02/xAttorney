...
const AdvocateCasePageTableComponent = () => {
  ...
  console.log('case details by id', viewCaseDetailsByID);
  ...
  const filterCourtNameList = () => {
    ...
    const newCaseCategoryId = JSON.stringify(caseCategoryId);
    const finalCaseCategoryId = newCaseCategoryId?.replace(
      /"([^"]+)":/g,
      '$1:'
    );
  ...
  const handleFirstFormSubmit = e => {
    ...
    // }
    // let data = { ...caseInfo };
    // if (caseInfo && caseInfo.wakalatnamaList && caseInfo.wakalatnamaList[0]) {
    //   data.wakalatnamaList[0].entryDate = new Date(
    //     data.wakalatnamaList[0].entryDate
    //   );
    // }
        ...
        customBodyRender: clientName => {
          return (
            <div>
              {clientName !== null
                ? clientName.map(k => {
                    if (!k || !k?.phoneList || k.phoneList.length === 0) {
                      return <span>No Phone Numberr</span>;
                    } else if (k && k?.phoneList && k?.phoneList.length > 0) {
