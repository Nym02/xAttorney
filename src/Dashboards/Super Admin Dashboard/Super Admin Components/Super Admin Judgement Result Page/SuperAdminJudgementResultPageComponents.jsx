...
import addJudgement from '../../../../assets/images/add-judgement.svg';
...
import SuperAdminJudgementResultPageTableComp from './SuperAdminJudgementResutlPageTableComp';

const SuperAdminJudgementResultPageComponents = () => {
  ...
  const onSubmit = (data, e) => {
    ...
    const createJudgementResultQuery = gql`
      mutation {
    ...
    axios
      .post(
        MAIN_API,
        {
          query: print(createJudgementResultQuery),
        },
        {
          headers: {
      ...
      .then(result => {
        const { createJudgmentResult } = result?.data?.data;
        if (createJudgmentResult !== null) {
          const { code, data, errors } =
            result?.data?.data.createJudgmentResult;

          if (code === 200 && data !== null) {
            addToast('Judgement result has been created successfully', {
              appearance: 'success',
              autoDismiss: true,
            });
  ...
  return (
    <>
      <DashboardPageHading title='Judgement Result' />
      <div className='flex flex-col space-y-3'>
        <div className='border-b border-deepIndigo border-opacity-50 flex items-end justify-between pb-1'>
          ...
          <button onClick={() => setOpen(true)} style={{ outline: 'none' }}>
            <img src={addJudgement} alt='' />
          </button>
        </div>
      </div>
      <div className='mt-7'>
        <SuperAdminJudgementResultPageTableComp />
      </div>
...
export default SuperAdminJudgementResultPageComponents;
