...
const SuperAdminClientBehalfPageTableComp = () => {
  const [anchorEl, setAnchorEl] = useState(null);
  const [open, setOpen] = useState(false);
  const [clientBehalfId, setClientBehalfId] = useState('');
  const [deleteModal, setDeleteModal] = useState(false);
  const { clientBehalf, setClientBehalf } = useContext(DataContext);
  const [clientBehalfUpdate, setClientBehalfUpdate] = useState([]);
  ...
  // -------------------------------- update client behalf data --------------------------------

  //getting update default value
  const viewDetailsByID = clientBehalf.find(({ id }) => id === clientBehalfId);
  console.log(viewDetailsByID);

  useEffect(() => {
    setClientBehalfUpdate(viewDetailsByID);
  }, [clientBehalfId, viewDetailsByID]);

  const handleUpdateDataChange = e => {
    setClientBehalfUpdate({
      ...clientBehalfUpdate,
  ...
  const onSubmit = (data, e) => {
    if (clientBehalfUpdate?.name === '' || clientBehalfUpdate?.name === null) {
      addToast('Client Behalf Name is Required', {
        appearance: 'error',
        autoDismiss: true,
      });
    } else {
      const updateClientBehalf = {
        id: clientBehalfId,
        name: clientBehalfUpdate?.name,
      };

      const onSuccessUpdateClientBehalf = result => {
        const { updateClientBehalf } = result?.data?.data;
        if (updateClientBehalf !== null) {
          const { code, data, errors } = result?.data?.data.updateClientBehalf;

          if (code === 200 && data !== null) {
            addToast('Client behalf has been updated successfully', {
              appearance: 'success',
              autoDismiss: true,
            });
      ...
      const onErrorUpdateClientBehalf = error => {
        addToast('Something wrong happend', {
          appearance: 'error',
          autoDismiss: true,
        });
      ...
      ApiHelper.clientBehalf
        .updateClientBehalf({ data: updateClientBehalf })
        .then(onSuccessUpdateClientBehalf)
        .catch(onErrorUpdateClientBehalf);

      handleClose();
    }
  };

  const deleteClientBehalf = () => {
    const onSuccessDeleteClientBehalf = () => {
      ApiHelper.clientBehalf
        .getClientBehalf()
        .then(res => {
          setClientBehalf(
          ...
          addToast('Client Behalf deleted successfully', {
            appearance: 'success',
            autoDismiss: true,
          });
        })
    ...
    const onErrorDeleteClientBehalf = error => {
      // console.log(error);
    };
    ...
    ApiHelper.clientBehalf
      ...
      .then(res => {
        return onSuccessDeleteClientBehalf();
      })
      .catch(onErrorDeleteClientBehalf);
  ...
  let newClientBehalf = [];
  clientBehalf.map((item, idx) => {
    newClientBehalf.push({
      sl: idx + 1,
      clientBehalfName: item?.name,
      id: item?.id,
    });
  });
        ...
        <ThemeProvider theme={theme}>
          <div className='w-full flex items-center justify-between space-x-8'>
            <TextField
              ...
              id='outlined-basic'
              name='name'
              value={clientBehalfUpdate ? clientBehalfUpdate?.name : ''}
  ...
  return (
    <>
      <MUIDataTable
        title={'Client Behalf List'}
        data={newClientBehalf}
        columns={columns}
        options={options}
      />
      <Modal
...
export default SuperAdminClientBehalfPageTableComp;
