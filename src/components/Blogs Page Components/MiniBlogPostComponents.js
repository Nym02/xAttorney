...
export default function MiniBlogPostComponents({
  title,
  date,
  img,
  comment,
  ...
  return (
    <>
      <Paper
        elevation={4}
        className=' w-full bg-lightSilver shadow-lg rounded sm:p-3 p-1 flex sm:flex-row flex-col sm:h-34 h-auto items-center cursor-pointer sm:space-y-0 space-y-3'
        onClick={() => window.open(`${link}`, '_blank')}
      >
        <div style={{ height: '103px', width: '113px' }}>
          <img src={img} alt='' />
        </div>
        <div className='text-primarydark space-y-2 sm:w-2/3 w-full'>
          ...
          <div className='flex justify-center sm:justify-start items-center sm:text-xsm text-xs space-x-2'>
